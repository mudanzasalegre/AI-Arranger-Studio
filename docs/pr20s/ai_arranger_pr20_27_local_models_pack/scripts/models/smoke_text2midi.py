...
def main() -> None:
    ...
    parser.add_argument("--prompt", default="A short hard bop jazz MIDI in C minor, 132 BPM, with piano, bass, drums and alto saxophone.")
    parser.add_argument("--allow-check-only", action="store_true", help="Only verify repo/checkpoints when inference wrapper is not implemented yet.")
    ...
    if not repo_dir.exists():
        ...
        if wrapper.exists():
            ...
            report["status"] = "ok" if completed.returncode == 0 and output.exists() and output.stat().st_size > 0 else "fail"
        elif args.allow_check_only:
            report["status"] = "check_only"
            report["note"] = "run_text2midi_inference.py not implemented yet; repo and checkpoints are present."
        else:
            report["status"] = "fail"
            report["error"] = "scripts/models/run_text2midi_inference.py is not implemented yet. Use --allow-check-only during PR-22 setup."
