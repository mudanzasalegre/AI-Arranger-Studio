...
def model_runtime_status() -> list[dict[str, object]]:
    return [
        ...
        {
            "id": "text2midi",
            "enabled": os.environ.get("AI_ENABLE_TEXT2MIDI", "false").lower() in {"1", "true", "yes"},
            "repo_dir_exists": Path(os.environ.get("TEXT2MIDI_REPO_DIR", "models/external_repos/text2midi")).exists(),
            "checkpoint_dir_exists": Path(os.environ.get("TEXT2MIDI_CHECKPOINT_DIR", "models/checkpoints/text2midi")).exists(),
        },
        {
            "id": "local_llm_planner",
            "enabled": os.environ.get("AI_ENABLE_LOCAL_LLM_PLANNER", "false").lower() in {"1", "true", "yes"},
